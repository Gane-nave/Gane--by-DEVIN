...
describe("Stripe Key Validation", () => {
  ...
  const testIfStripeConfigured = sk && pk ? it : it.skip;

  testIfStripeConfigured("should have Stripe Secret Key set", () => {
    expect(sk).toBeTruthy();
    expect(sk!.startsWith("sk_test_") || sk!.startsWith("sk_live_")).toBe(true);
  });

  testIfStripeConfigured("should have Stripe Publishable Key set", () => {
    expect(pk).toBeTruthy();
    expect(pk!.startsWith("pk_test_") || pk!.startsWith("pk_live_")).toBe(true);
  });

  testIfStripeConfigured("should connect to Stripe API successfully", async () => {
