...
/// `GANE_*` environment variable overrides.
...
/// Apply `GANE_*` environment variable overrides.
///
/// Supported overrides:
/// - `GANE_API_PORT` → `api.port`
/// - `GANE_API_HOST` → `api.host`
/// - `GANE_LOG_LEVEL` → `system.log_level`
/// - `GANE_WORKER_THREADS` → `system.worker_threads`
/// - `GANE_INSTANCE_NAME` → `system.instance_name`
/// - `GANE_GNSS_MIN_SATELLITES` → `gnss.min_satellites`
/// - `GANE_TELEMETRY_BUFFER` → `telemetry.buffer_capacity`
pub fn apply_env_overrides(config: &mut AuroraConfig) -> Result<(), ConfigError> {
    if let Ok(val) = std::env::var("GANE_API_PORT") {
        config.api.port = val
            .parse()
            .map_err(|_| ConfigError::EnvOverride("GANE_API_PORT must be a u16".into()))?;
    }
    if let Ok(val) = std::env::var("GANE_API_HOST") {
        config.api.host = val;
    }
    if let Ok(val) = std::env::var("GANE_LOG_LEVEL") {
    ...
    if let Ok(val) = std::env::var("GANE_GNSS_MIN_SATELLITES") {
        config.gnss.min_satellites = val.parse().map_err(|_| {
            ConfigError::EnvOverride("GANE_GNSS_MIN_SATELLITES must be a usize".into())
        })?;
    }
    if let Ok(val) = std::env::var("GANE_TELEMETRY_BUFFER") {
        config.telemetry.buffer_capacity = val.parse().map_err(|_| {
            ConfigError::EnvOverride("GANE_TELEMETRY_BUFFER must be a usize".into())
        })?;
    }
    Ok(())
}
